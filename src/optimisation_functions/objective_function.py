...
def objective_function(racing_line, ):
    
    # Penalty for deviating too far from the track boundaries
    boundary_penalty = np.mean(np.linalg.norm(racing_line - (inner_bounds + outer_bounds) / 2, axis=1))
    ...
    boundary_penalty /= np.max(np.linalg.norm(outer_bounds - inner_bounds, axis=1))
    smoothness_penalty /= np.max(np.abs(np.diff(inner_bounds, axis=0)))
    max_cornering_speed /= np.max(cornering_speed)
    
    # Combine penalties and cornering speed into an overall score
    score = 1 - (0.3 * boundary_penalty + 0.3 * smoothness_penalty + 0.4 * (1 - max_cornering_speed))
